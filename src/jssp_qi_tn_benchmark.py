...
def load_jobs_data(instance_name):
    ...
    This means job 0 has operations:
        ...
        etc.
    """
    if load_benchmark_instance is None:
        raise ImportError(
            "job_shop_lib is required for standalone TN benchmark instance loading. "
            "Install it with: python -m pip install job-shop-lib"
        )
...
def plot_metric(results, instance_order, metric, title, ylabel, logy=False):
    """
...
def main():
    # Install the optional standalone loader dependency only for this script mode.
    global load_benchmark_instance

    install_if_missing("job_shop_lib", "job-shop-lib")
    if load_benchmark_instance is None:
        from job_shop_lib.benchmarking import load_benchmark_instance as loader

        load_benchmark_instance = loader
